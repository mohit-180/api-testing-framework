...
class ConfigLoader:
    ...
    def _validate(self, data: Dict[str, Any]) -> None:
        ...
        if not isinstance(data["base_url"], str) or not data["base_url"].strip():
            raise ValueError("Config field 'base_url' must be a non-empty string.")

        if not isinstance(data["endpoints"], list) or len(data["endpoints"]) == 0:
            raise ValueError("Config field 'endpoints' must be a non-empty list.")

        valid_methods = {"GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"}

        for idx, ep in enumerate(data["endpoints"]):
            if not isinstance(ep, dict):
                raise ValueError(f"Endpoint at index {idx} must be an object.")

            if "path" not in ep or not isinstance(ep["path"], str):
                raise ValueError(f"Endpoint at index {idx} missing valid 'path'.")

            method = ep.get("method", "GET").upper()
            if method not in valid_methods:
                raise ValueError(f"Unsupported HTTP method '{method}' for endpoint '{ep.get('path')}'.")
            ep["method"] = method
        ...
        if data["concurrent_users"] <= 0:
            raise ValueError("'concurrent_users' must be greater than 0.")

        if data["number_of_requests"] <= 0:
            raise ValueError("'number_of_requests' must be greater than 0.")
